...
contract SmartVaultV3 is ISmartVault {
    ...
    function getAssets() private view returns (Asset[] memory) {
        ...
        for (uint256 i = 0; i < acceptedTokens.length; i++) {
            ...
            assets[i] = Asset(
                token,
                assetBalance,
                calculator.tokenToEurAvg(token, assetBalance)
            );
        }
        return assets;
    }
    ...
    function removeAsset(
        ...
        ITokenManager.Token memory token = getTokenManager().getTokenIfExists(
            _tokenAddr
        );
    ...
    function mint(
        address _to,
        uint256 _amount
    ) external onlyOwner ifNotLiquidated {
        uint256 fee = (_amount * ISmartVaultManagerV3(manager).mintFeeRate()) /
    ...
    function burn(uint256 _amount) external ifMinted(_amount) {
        uint256 fee = (_amount * ISmartVaultManagerV3(manager).burnFeeRate()) /
            ISmartVaultManagerV3(manager).HUNDRED_PC();
    ...
    function executeNativeSwapAndFee(
        ...
        (bool sent, ) = payable(ISmartVaultManagerV3(manager).protocol()).call{
            value: _swapFee
        }("");
        require(sent, "err-swap-fee-native");
        ISwapRouter(ISmartVaultManagerV3(manager).swapRouter2())
            .exactInputSingle{value: _params.amountIn}(_params);
    }
    ...
    function executeERC20SwapAndFee(
        ...
        IERC20(_params.tokenIn).safeApprove(
            ISmartVaultManagerV3(manager).swapRouter2(),
            _params.amountIn
        );
        ISwapRouter(ISmartVaultManagerV3(manager).swapRouter2())
