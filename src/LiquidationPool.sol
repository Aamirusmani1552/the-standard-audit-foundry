...
contract LiquidationPool is ILiquidationPool {
    ...
    function holderPendingStakes(
        address _holder
    ) private view returns (uint256 _pendingTST, uint256 _pendingEUROs) {
        for (uint256 i = 0; i < pendingStakes.length; i++) {
        ...
        returns (Position memory _position, Reward[] memory _rewards)
    {
        ...
        (uint256 _pendingTST, uint256 _pendingEUROs) = holderPendingStakes(
            _holder
        );
    ...
    function distributeFees(uint256 _amount) external onlyManager {
        ...
        if (tstTotal > 0) {
            ...
            for (uint256 i = 0; i < holders.length; i++) {
                ...
                positions[_holder].EUROs +=
                    (_amount * positions[_holder].TST) /
                    tstTotal;
            }
    ...
    // @audit what does this function do?
    ...
    function distributeAssets(
        ...
        // @audit this only hold total stakes in the form of tst or euro or both. how is it calculated for he rewards because both represent different assets
