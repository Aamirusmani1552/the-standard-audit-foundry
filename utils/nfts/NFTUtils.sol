...
library NFTUtils {
    ...
    function padFraction(
        bytes memory _input,
        uint8 _dec
    ) private pure returns (bytes memory fractionalPartPadded) {
        ...
        while (i > 0) {
            ...
            if (j > 0) {
                ...
                if (_input[j] != bytes1("0") || smallestCharacterAppended) {
                    ...
                    fractionalPartPadded = new bytes(
                        fractionalPartPadded.length - 1
                    );
                }
            } else {
    ...
    function truncateFraction(
        bytes memory _input,
        uint8 _places
    ) private pure returns (bytes memory truncated) {
    ...
    function toDecimalString(
        ...
        bytes memory fractionalPartPadded = padFraction(
            fractionalPart,
            _inputDec
        );
        if (fractionalPartPadded.length > maxDecPlaces)
            fractionalPartPadded = truncateFraction(
                fractionalPartPadded,
                maxDecPlaces
            );
